...
```bash
conda create python=3.8 -n ejercicio_django
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate ejercicio_django
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///ejercicio_django` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE ejercicio_django WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE ejercicio_django TO ;
```
...
ERROR:  database "ejercicio_django" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists ejercicio_django
...
```bash
dropdb --if-exists ejercicio_django
